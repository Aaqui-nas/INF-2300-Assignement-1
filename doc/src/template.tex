...
\texttt{text}. The server always assigns the \texttt{id}; I never trust
one from the client, since it is what identifies a message across
requests, and letting a client choose it would risk collisions with other
messages. I address a message in two ways: through the request body (an
...
I keep the router simple: two Python dictionaries and a few decorator
...
every route in this assignment: a few fixed static paths, the
\texttt{/messages} collection, and the one dynamic route,
\texttt{/messages/<id>}.

\subsection{Request parsing}\label{Subsection:impl_parsing}
I read the request line and headers line by line with
\texttt{rfile.readline()}. I tolerate one leading blank line, as RFC 9112
section 3 allows, then require a well-formed request line. Any malformed
...
\texttt{/favicon.ico}, \texttt{/test.txt}) resolves to one fixed filename
inside \texttt{BASE\_DIR}, so a request for one of them can never escape
that directory. Any \texttt{GET} request that matches none of these exact
routes falls through to a generic fallback. I resolve the target path with
\texttt{os.path.realpath()} and check it against \texttt{BASE\_DIR} before
touching the filesystem; anything outside \texttt{BASE\_DIR} gets
\texttt{403}, before I even check whether the file exists. A file that
exists but is not whitelisted, such as \texttt{server.py}, also gets
\texttt{403} rather than \texttt{404}. This way, a client cannot tell a
forbidden file from a missing one.
...
helpers. I also register \texttt{POST /messages/<id>}, only to answer
\texttt{403}: creating a message only makes sense on the collection
endpoint. Every change goes through \texttt{save\_messages()}, which
writes the full state to \texttt{messages.json}.
...
\subsection{Automated testing}\label{Subsection:results_automated}
The provided \texttt{test\_client.py} drives the handler through a second,
in-process server. It checks 12 behaviours: a valid response code, the
body and headers of \texttt{GET /}, the \texttt{404} and \texttt{403}
status codes, protection against directory traversal, and the behaviour
of \texttt{POST} to \texttt{/test.txt}. It runs the suite in sequential
order, then in random order; the random pass only runs if the sequential
pass fully succeeds. I applied the errata fix for this file before running
it: every request target now starts with a slash. All 12 tests passed, in
both sequential and random order (12/12, 100\%).
...
Section \ref{Section:Discussion}: a missing \texttt{text} or \texttt{id}
field, a non-existent id, malformed JSON, an empty body, a client-supplied
id being ignored, \texttt{POST} to an id URL being forbidden, and a
created message being readable back through \texttt{load\_messages()}.
